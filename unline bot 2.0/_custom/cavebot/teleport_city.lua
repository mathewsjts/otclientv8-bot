...
CaveBot.Extensions.TeleportCity.setup = function()
  CaveBot.registerAction("TeleportCity", "#ffffff", function(value)
    local cities_coordinates = {
      ...
      ["farmine"] = {
        {x=33025, y=31553, z=10},
        {x=33025, y=31553, z=14},
        {x=33025, y=31553, z=12}
      },
    ...
    local city_coordinates = cities_coordinates[city_name]
    ...
    if city_name == "farmine" then
      local near_any_farmine = false
      for _, coordinate in ipairs(city_coordinates) do
        if player_position.z == coordinate.z and getDistanceBetween(player_position, coordinate) <= max_distance then
          near_any_farmine = true
          break
        end
      end
      if not near_any_farmine then
        NPC.say("!tp " .. city_name)
        return "retry"
      end
    else
      if player_position.z ~= city_coordinates.z or getDistanceBetween(player_position, city_coordinates) > max_distance then
        NPC.say("!tp " .. city_name)
        return "retry"
      end
    end
