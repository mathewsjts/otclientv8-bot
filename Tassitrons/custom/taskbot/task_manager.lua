...
-- local signal = "onEditText"
-- local object = g_game

-- local slot = function(id, itemId, maxLength, text, writer, time)
--   local playerName = TaskBot.getCharWithIncompleteTasks() or player:getName()
--   g_game.editText(id, playerName)
-- end

-- if type(g_game[signal]) == 'function' then
--   object[signal] = { object[signal] }
-- end

-- if type(object[signal]) == 'table' and table.size(object[signal]) < 2 then
  ...
  for _, container in pairs(getContainers()) do
    if container:getName() == "wardrobe" or container:getName() == "treasure chest" or container:getContainerItem():getId() == 3504 then
      for i, item in pairs(container:getItems()) do
        ...
        -- g_game.move(item, , item:getCount())
...
TaskBot.loginNextCharacter = function()
  ...
  for i=1, #children do
    ...
    -- say(children[i].characterName .. tostring(i))
    -- if children[i].characterName == "Fiora" then
    --   characterList:focusChild(children[i], 1)
    --   -- characterList:ensureChildVisible(children[i])
    --   -- addEvent(function() charactersWindow:ensureChildVisible(focusLabel) end)
    ...
    --   -- g_game["onLogout"] = function()
    --   --   onLogout()
    --   --   -- children[i]["onDoubleClick"][#children[i]["onDoubleClick"]]()
    --   -- end

    --   g_game.safeLogout()
    --   break
    -- end
    -- if children[i]["onDoubleClick"] then
      -- charactersWindow:show()
      -- say(type(children[i]["onDoubleClick"][1]))
    -- end
  end
end
