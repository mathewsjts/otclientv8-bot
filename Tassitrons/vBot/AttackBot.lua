...
macro(100, function()
  ...
  for i, child in ipairs(panel.entryList:getChildren()) do
    ...
    if entry.enabled and manapercent() >= entry.mana then
      if (type(attackData) == "string" and canCast2(entry.spell, not currentSettings.ignoreMana, not currentSettings.Cooldown)) or (entry.itemId > 100 and (not currentSettings.Visible or findItem(entry.itemId))) then
        -- first PVP scenario
