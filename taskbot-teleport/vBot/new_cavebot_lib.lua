...
function CaveBot.ConversationList(expressions)
    ...
    local talkDelay = 0
    for i, expr in ipairs(expressions) do
        schedule(talkDelay, function() NPC.say(expr:trim()) end)
        talkDelay = talkDelay + delay
    end
end

function CaveBot.ConversationDelay(delay_value)
    local delay = storage.extras.talkDelay or 1000
    CaveBot.delay(CaveBot.Config.get("ping") + ((delay_value + 2) * delay))
end
