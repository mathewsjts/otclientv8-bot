...
CaveBot.Editor.setup = function()
  ...
  onPlayerPositionChange(function(pos)
    ...
    if not CaveBot.actionList:getFocusedChild() then return end
    local text = CaveBot.actionList:getFocusedChild():getText()
    -- say(text)
    ...
    if action and value and action == "goto" then

      local coords = string.split(value, ",")
      if #coords > 2 then
        if math.abs(pos.x - tonumber(coords[1])) + math.abs(pos.y - tonumber(coords[2])) >= 40 then
          ui.pos:setColor("red")
          return
        end
      end
